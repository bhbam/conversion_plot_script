...
decay = "IMG_aToTauTau_Hadronic_tauDR0p4_mass_below1p2_Gev_dataset_2_unbaised_unphysical"
local="/eos/uscms/store/group/lpcml/bbbam/IMG/IMG_aToTauTau_Hadronic_tauDR0p4_m1p2To3p6_dataset_2_unbaised_unphysical/IMG_aToTauTau_Hadronic_tauDR0p4_m1p2To3p6_dataset_2_unbaised_%s_train_unphysical.parquet"%subset
out_dir="/eos/uscms/store/group/lpcml/bbbam/IMG/%s"%decay
out_dir_plots = "plots_from_mass_below_1.2_Gev_unbiased_unphysical_pq_to_pq/%s"%(subset)
...
file_str = "%s/IMG_aToTauTau_Hadronic_tauDR0p4_mass_below_1p2_Gev_dataset_2_unbaised_%s_train_unphysical.parquet"%(out_dir,subset)
...
print('>> Writing data...')
flag_for = np.full((len(mass_bins), len(pt_bins)), False)
table = True
for i, idx in enumerate(idxs[start:stop]):
    ...
    if mass_ > 1.6 : continue
    pt_bin_index = np.digitize(np.squeeze(pt_), pt_bins) - 1
    index = 1
    while mass_bins[mass_bin_index-index] > mass_bins[0]:
