...
def merge_samples(dset, start, stop, idxs, decay):
    ...
    for i, idx in enumerate(idxs[start:stop]):
        ...
        # if decay =='signals_Tau_classifier':
        #     data['y']     = 1.
        # else:
        #     data['y']     = 0.
...
# subfile_name = 'DYToTauTau_M-50_13TeV_train_1'
# subfile_name = 'DYToTauTau_M-50_13TeV_train_2'
# subfile_name = 'DYToTauTau_M-50_13TeV_train_3'
...
# decay='background_Tau_classifier' # name inse IMG directory
decay='signals_Tau_classifier' # name inse IMG directory
# decay='background_Tau_classifier'
...
files_ = '%s/DYToTauTau*.parquet*'%local
fs = glob.glob(files_)
...
sort_nicely(fs)
# print(fs)
# dset = ConcatDataset([ParquetDatasetTable(fs[i]) for i in range(0,15)]) # train set 1
# dset = ConcatDataset([ParquetDatasetTable(fs[i]) for i in range(26,42)]) # train set 2
# dset = ConcatDataset([ParquetDatasetTable(fs[i]) for i in range(51,76)]) # train set 3
dset = ConcatDataset([ParquetDatasetTable(fs[i]) for i in range(76,90)]) # validation
nevts_in = len(dset)
...
# start, stop = 0, 425000 # train set
start, stop = 0, 255000 #validation
