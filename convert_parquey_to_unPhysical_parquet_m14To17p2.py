...
decay = "IMG_aToTauTau_Hadronic_tauDR0p4_m14To17p2_dataset_2_unbaised_unphysical"
...
out_dir_plots = "plots_from_m14To17p2_unbiased_unphysical_pq_to_pq/%s"%(subset)
...
file_str = "%s/IMG_aToTauTau_Hadronic_tauDR0p4_m14To17p2_dataset_2_unbaised_%s_train_unphysical.parquet"%(out_dir,subset)
...
for i, idx in enumerate(idxs[start:stop]):
    ...
    m_unphy.append(mass_)
    pt_unphy.append(pt_)
    if bin_count > event_per_bin or flag_for[mass_bin_index,pt_bin_index]: continue
    flag_for[mass_bin_index,pt_bin_index] = True
    for n in range(int(event_per_bin - bin_count)):
        mass_new = random.uniform(mass_bins[mass_bin_index], mass_bins[mass_bin_index]+0.4)
        data = {}
        data['X_jet'] = np.squeeze(t["X_jet"])*mass_new/mass_
        data['am']    = mass_new
