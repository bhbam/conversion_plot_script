...
event_per_bin =700 # number of events you wabt to mass pt bins.
...
out_dir_plots = "plots_from_pq_unbiased/%s"%decay
...
sub_file = [n0, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10]



# chunks = [(sub_file[n],sub_file[n+1]) for n in range(range(len(sub_file)-1))]
# def main_function():
#     files = []
#     print("Processing root files from %d -> %d "%(sub_file[n],sub_file[n+1]))
#     for i in range (sub_file[n],sub_file[n+1]):
#         files.append(rhFileList[i])
#
#
#
#     # for file in rhFileList:
#     #     files.append(file)
#
#     # print(files)
#
...
#             os.makedirs(out_dir)
#     outStr ='%s/%s_%d.parquet'%(out_dir, decay, n)
...
#
#     writer.close()
# pool = multiprocessing.Pool(processes = len(sub_file)-1)
# pool.starmap(main_function, chunks)


for n in range(len(sub_file)-1):
