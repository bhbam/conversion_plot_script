...
parser.add_argument('-u', '--unphysical_samples', default=0, type=int, help='flag for real and unphysical samples')
...
with h5py.File(f'{outStr}', 'w') as proper_data:
        ...
        datasets = {
            name: proper_data.create_dataset(
                name,
                shape= (0, 5, 125, 125) if 'all_jet' in name else (0,1),
                maxshape=(None, 5, 125, 125) if 'all_jet' in name else (None, 1),
                dtype='float32',  # Specify an appropriate data type
                compression='lzf',
                chunks=(chunk_size, 5, 125, 125) if 'all_jet' in name else (chunk_size, 1),
        ...
        for iEvt in range(iEvtStart,iEvtEnd):
            ...
            ECAL_energy = np.array(rhTree.ECAL_energy_AOD).reshape(280,360)
            HBHE_energy = np.array(rhTree.HBHE_energy_AOD).reshape(56,72)
            ...
            X_CMSII            = np.stack([TracksAtECAL_pt, TracksAtECAL_dZSig, TracksAtECAL_d0Sig, ECAL_energy, HBHE_energy], axis=0) # (13, 280, 360)




            for name, dataset in datasets.items():
                dataset.resize((end_idx,5, 125, 125) if 'all_jet' in name else (end_idx,1))

            for i in range(ys):
                proper_data['all_jet'][end_idx - ys + i, :, :, :] = crop_jet(X_CMSII, iphis[i], ietas[i], jet_shape=125)
                if args.unphysical_samples ==1:
                    proper_data['am'][end_idx - ys + i, :] = new_mass(ams[i])
                else:
                    proper_data['am'][end_idx - ys + i, :] = ams[i]
                proper_data['apt'][end_idx - ys + i, :] = apts[i]
                proper_data['jet_mass'][end_idx - ys + i, :] = jetms[i]
